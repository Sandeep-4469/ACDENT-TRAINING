#!/usr/bin/env python3
"""
Train HeatmapNet with ResNet18 backbone on DSAA Dental dataset (5-fold CV).
...
Usage:
  ...
  python train_resnet18.py --k-folds 5 --epochs 300 --batch-size 8

Outputs (per fold + overall):
  results/train_resnet18/<run_name>/
    fold_1/  ...  fold_5/
...
DEFAULT_SAVE_ROOT = DSAA_ROOT / "results" / "train_resnet18"
...
class HeatmapNet(nn.Module):
    def __init__(self):
        ...
        self.backbone = nn.Sequential(*list(backbone.children())[:-2])  # output: [B, 512, 16, 16]
...
# ---------------------------------------------------------------------------
# Utilities  (identical to train_resnet50.py)
# ---------------------------------------------------------------------------
...
def arc_losses(
    ...
    for idx_a, idx_b, mask_count in [(2, 3, 6), (4, 5, 6), (10, 11, 14), (12, 13, 14)]:
        pass
    ...
    def _update(mask, ci, cj, gi, gj):
        ...
        n = mask.sum() + 1e-6
        side_order_loss   = side_order_loss   + (side    * mask).sum() / n
        center_align_loss = center_align_loss + (align   * mask).sum() / n
        contrast_loss     = contrast_loss     + (contrast * mask).sum() / n
        ...
        for imgs, _, gt_pts, keys in tqdm(loader, leave=False):
            ...
            for i in range(len(imgs)):
                ...
                if valid_kps == 6:
                    ...
                    l_err = abs(pred_arcs[0][1] - gt_arcs[0][1])
                    r_err = abs(pred_arcs[1][1] - gt_arcs[1][1])
...
def train_one_fold(fold_name: str, fold_root: Path, save_root: Path, seed: int) -> Dict[str, float]:
    ...
    with (fold_save / "run_config.json").open("w") as f:
        json.dump({"fold": fold_name, "backbone": "resnet18", "seed": seed,
                   ...
                   "arc_margin_px": arc_margin_px, "img_size": IMG_SIZE, "heatmap_size": HEATMAP_SIZE,
                   "num_kps": NUM_KPS, "device": str(DEVICE),
...
def plot_loss_curves(all_epochs_csv: Path, out_png: Path) -> None:
    ...
    fig.suptitle("ResNet18 — DSAA Dental", fontsize=12)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train HeatmapNet (ResNet18) on DSAA Dental — 5-fold CV.")
...
def main() -> None:
    ...
    if summary_rows:
        ...
        with (run_root / "best_model_overall.json").open("w") as f:
            json.dump({"best_fold": best_fold, "backbone": "resnet18",
    ...
    with (run_root / "run_artifacts.json").open("w") as f:
        json.dump({"run_root": str(run_root), "backbone": "resnet18",
