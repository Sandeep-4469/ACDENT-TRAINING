#!/usr/bin/env python3
"""
Train HeatmapNet with ConvNeXt-Tiny backbone on DSAA Dental dataset (5-fold CV).

Prerequisites:
  Run prepare_folds.py first to create the 5-fold dataset with augmentation.

Usage:
  python train_convnext_tiny.py
  python train_convnext_tiny.py --k-folds 5 --epochs 300 --batch-size 8

Outputs (per fold + overall):
  results/train_convnext_tiny/<run_name>/
    fold_1/  ...  fold_5/
      training_log.csv  run_config.json  heatmap_best.pth  heatmap_last.pth  test_predictions.json
    kfold_summary.csv  all_folds_training_log.csv  loss_curves.png
    best_model_overall.pth  best_model_overall.json  run_artifacts.json
"""
...
DEFAULT_SAVE_ROOT = DSAA_ROOT / "results" / "train_convnext_tiny"
...
class HeatmapNet(nn.Module):
    def __init__(self):
        ...
        self.backbone = backbone.features  # output: [B, 768, 16, 16] for 512x512 input
...
# ---------------------------------------------------------------------------
# Utilities  (identical to other train scripts)
# ---------------------------------------------------------------------------
...
def estimate_arc_margin_px(
    ...
    return float(np.clip(0.12 * min(pools), 6.0, 14.0)) if pools else default_margin
...
def arc_losses(
    ...
    def _update(mask, ci, cj, gi, gj):
        nonlocal side_order_loss, center_align_loss, contrast_loss
        ...
        ps, _ = torch.sort(pred_arcs, dim=1); gs, _ = torch.sort(gt_arcs, dim=1)
        ...
        contrast = F.relu(d_lo+arc_margin_px-d_la) + F.relu(d_ro+arc_margin_px-d_ra)
        n = mask.sum() + 1e-6
        side_order_loss   += (side    * mask).sum() / n
        center_align_loss += (align   * mask).sum() / n
        contrast_loss     += (contrast * mask).sum() / n

    max_mask = (gt_pts[:, :, 0] >= 0).sum(dim=1) == 6
    _update(max_mask, 2, 3, 4, 5)
    man_mask = (gt_pts[:, :, 0] >= 0).sum(dim=1) == 14
    _update(man_mask, 10, 11, 12, 13)
...
def evaluate(model: nn.Module, loader: DataLoader, pred_json_path: Path) -> Dict[str, float]:
    ...
    with torch.no_grad():
        for imgs, _, gt_pts, keys in tqdm(loader, leave=False):
            ...
            for i in range(len(imgs)):
                ...
                pred_pts    = [[x * IMG_SIZE / HEATMAP_SIZE, y * IMG_SIZE / HEATMAP_SIZE] for x, y in pred_coords]
                gt          = gt_pts[i].numpy()
                ...
                if valid_kps == 6:
                    ...
                    l_err = abs(pred_arcs[0][1]-gt_arcs[0][1]); r_err = abs(pred_arcs[1][1]-gt_arcs[1][1])
                    errors_px["left_arc"].append(l_err); errors_px["right_arc"].append(r_err)
                    if px_to_mm:
                        errors_mm["left_arc"].append(l_err*px_to_mm); errors_mm["right_arc"].append(r_err*px_to_mm)
                else:
                    inc_err = abs(sum(line_len(pred_pts[2*li], pred_pts[2*li+1]) for li in [1,2,3,4]) -
                                  sum(line_len(gt[2*li],       gt[2*li+1])       for li in [1,2,3,4]))
                    errors_px["incisor_sum"].append(inc_err)
                    if px_to_mm: errors_mm["incisor_sum"].append(inc_err*px_to_mm)
                    pred_arcs, gt_arcs = [], []
                    for li in [5, 6]:
                        p1, p2 = pred_pts[2*li], pred_pts[2*li+1]
                        g1, g2 = gt[2*li],       gt[2*li+1]
    ...
    for k, vals in errors_px.items():
        if vals: means[f"{k}_px"] = float(np.mean(vals))
    for k, vals in errors_mm.items():
        if vals: means[f"{k}_mm"] = float(np.mean(vals))
...
# Training — one fold
# ---------------------------------------------------------------------------
def train_one_fold(fold_name: str, fold_root: Path, save_root: Path, seed: int) -> Dict[str, float]:
    ...
    for p in [train_img_dir, train_ann_file, test_img_dir, test_ann_file]:
        if not Path(p).exists():
            raise FileNotFoundError(f"Missing: {p}. Run prepare_folds.py first.")
    ...
    with (fold_save / "run_config.json").open("w") as f:
        json.dump({"fold": fold_name, "backbone": "convnext_tiny", "seed": seed,
                   "train_img_dir": str(train_img_dir), "train_ann_file": str(train_ann_file),
                   "test_img_dir": str(test_img_dir), "test_ann_file": str(test_ann_file),
                   "save_dir": str(fold_save), "batch_size": BATCH_SIZE, "epochs": EPOCHS,
                   "lr": LR, "weight_decay": WEIGHT_DECAY, "patience": PATIENCE,
                   "arc_margin_px": arc_margin_px, "img_size": IMG_SIZE,
                   "heatmap_size": HEATMAP_SIZE, "num_kps": NUM_KPS, "device": str(DEVICE),
                   "train_samples": len(train_ds), "test_samples": len(test_ds)}, f, indent=2)
    ...
    grid_y, grid_x = torch.meshgrid(torch.arange(HEATMAP_SIZE, device=DEVICE),
    ...
    for epoch in range(EPOCHS):
        ...
        for imgs, heatmaps, gt_pts, _ in train_loader:
            ...
            length_loss = line_count = torch.tensor(0.0, device=DEVICE)
            ...
            sl, al, cl = arc_losses(coord_x, gt_pts, arc_margin_px)
            loss = (heatmap_loss + COORD_LOSS_W*coord_loss + LENGTH_LOSS_W*length_loss
                    + ARC_SIDE_LOSS_W*sl + ARC_CENTER_ALIGN_W*al + ARC_CONTRAST_W*cl)
            loss.backward()
...
def plot_loss_curves(all_epochs_csv: Path, out_png: Path) -> None:
    ...
    with all_epochs_csv.open("r", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            try:
                rows.append({"fold": row.get("fold",""), "epoch": int(row.get("epoch",0)),
    ...
    fig.suptitle("ConvNeXt-Tiny — DSAA Dental", fontsize=12)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train HeatmapNet (ConvNeXt-Tiny) on DSAA Dental — 5-fold CV.")
...
def main() -> None:
    ...
    for i in range(1, args.k_folds + 1):
        ...
        print(f"\n{'='*60}\nTraining {fold_name} — ConvNeXt-Tiny\n{'='*60}")
        metrics = train_one_fold(fold_name, fold_root, run_root, seed=args.seed + i)
        summary_rows.append(metrics); print(f"[{fold_name}] Metrics: {metrics}")
    ...
    with summary_csv.open("w", newline="") as f:
        ...
        w.writeheader()
        for row in summary_rows: w.writerow({k: row.get(k, "") for k in fields})
    ...
    with all_epochs_csv.open("w", newline="", encoding="utf-8") as fout:
        ...
        for i in range(1, args.k_folds + 1):
            ...
            if not fold_log.exists(): continue
    ...
    print(f"\n{'='*60}\nPER-FOLD RESULTS — ConvNeXt-Tiny\n{'='*60}")
    for row in summary_rows:
        print(f"{row.get('fold')}: best_val={row.get('best_val_loss',float('nan')):.4f} | "
    ...
    if summary_rows:
        has_sum3 = all("sum_incisor_left_right_mm" in r and r["sum_incisor_left_right_mm"] != "" for r in summary_rows)
        if has_sum3:
            ...
            cval      = float(best_row["sum_incisor_left_right_mm"])
        else:
            best_row  = min(summary_rows, key=lambda r: float(r["best_val_loss"]))
            criterion = "best_val_loss (lower is better)"
            cval      = float(best_row["best_val_loss"])
        best_fold = str(best_row["fold"])
        src = run_root / best_fold / "heatmap_best.pth"
        dst = run_root / "best_model_overall.pth"
        if src.exists(): shutil.copy2(src, dst)
        with (run_root / "best_model_overall.json").open("w") as f:
            json.dump({"best_fold": best_fold, "backbone": "convnext_tiny",
                       "selection_criterion": criterion, "criterion_value": cval,
                       "source_model": str(src), "saved_model": str(dst)}, f, indent=2)
        print(f"\nBest model: {dst} (from {best_fold}, {criterion}={cval:.6f})")

    fold_artifacts = [{"fold": f"fold_{i}", "fold_dir": str(run_root / f"fold_{i}"),
                       "model_best": str(run_root / f"fold_{i}" / "heatmap_best.pth"),
                       "model_last": str(run_root / f"fold_{i}" / "heatmap_last.pth")}
                      for i in range(1, args.k_folds + 1)]
    with (run_root / "run_artifacts.json").open("w") as f:
        json.dump({"run_root": str(run_root), "backbone": "convnext_tiny",
                   "kfold_summary_csv": str(summary_csv),
                   "best_model_overall_pth": str(run_root / "best_model_overall.pth"),
                   "fold_artifacts": fold_artifacts}, f, indent=2)
