#!/usr/bin/env python3
"""
Train HeatmapNet with HRNet-W32 backbone on DSAA Dental dataset (5-fold CV).

HRNet-W32 is not a Sequential model — it maintains multiple resolution branches
in parallel throughout the network, so the forward pass must be written explicitly
by copying the individual stage modules from the pretrained model.

Prerequisites:
  Run prepare_folds.py first to create the 5-fold dataset with augmentation.

Usage:
  python train_hrnet_w32.py
  python train_hrnet_w32.py --k-folds 5 --epochs 300 --batch-size 8

Outputs (per fold + overall):
  results/train_hrnet_w32/<run_name>/
...
DEFAULT_SAVE_ROOT = DSAA_ROOT / "results" / "train_hrnet_w32"
...
# ---------------------------------------------------------------------------
# Model — HRNet-W32 backbone
# HRNet is not Sequential; each stage runs parallel resolution branches.
# We copy individual stage modules and write an explicit forward pass.
# The pretrained classification head (incre/downsamp/final_layer) aggregates
# branches into a single (B, 2048, H/32, W/32) feature map, matching ResNet-50.
# ---------------------------------------------------------------------------
class HeatmapNet(nn.Module):
    def __init__(self):
        super().__init__()
        hrnet = models.hrnet_w32(weights=models.HRNet_W32_Weights.DEFAULT)

        # Stem: conv1 → bn1 → relu → maxpool (stride 4 total)
        self.stem = nn.Sequential(hrnet.conv1, hrnet.bn1, hrnet.relu, hrnet.maxpool)
        self.layer1 = hrnet.layer1

        # Multi-resolution stages and their transition layers
        ...
        # Classification head — fuses multi-resolution branches to 2048-d
        self.incre_modules   = hrnet.incre_modules
        self.downsamp_modules = hrnet.downsamp_modules
        self.final_layer     = hrnet.final_layer  # outputs (B, 2048, 16, 16) for 512x512

        # Number of branches per stage (used in forward)
        self._nb2 = len(hrnet.transition1)
        self._nb3 = len(hrnet.transition2)
        self._nb4 = len(hrnet.transition3)

        # Deconv head shared with other backbone scripts
        ...
        self.final_head = nn.Conv2d(256, NUM_KPS, 1)

    def forward(self, x):
        x = self.stem(x)
        x = self.layer1(x)

        # Stage 2: create two resolution branches
        x_list = []
        for i in range(self._nb2):
        ...
        # Stage 3: extend to three resolution branches
        x_list = []
        for i in range(self._nb3):
        ...
        # Stage 4: extend to four resolution branches
        x_list = []
        for i in range(self._nb4):
        ...
        # Fuse branches: incre_modules increase channel depth per branch,
        # then downsamp_modules halve spatial size and add to the next branch.
        y = [self.incre_modules[i](y_list[i]) for i in range(len(y_list))]
        for i in range(len(self.downsamp_modules)):
            y[i + 1] = self.downsamp_modules[i](y[i]) + y[i + 1]

        # final_layer maps the fused tensor to 2048 channels
        feat = self.final_layer(y[-1])  # (B, 2048, 16, 16) for 512x512 input

        return self.final_head(self.deconv(feat))
...
def plot_loss_curves(all_epochs_csv: Path, out_png: Path) -> None:
    ...
    fig.suptitle("HRNet-W32 — DSAA Dental", fontsize=12)
    fig.tight_layout()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train HeatmapNet (HRNet-W32) on DSAA Dental — 5-fold CV.")
...
def main() -> None:
    ...
    for i in range(1, args.k_folds + 1):
        ...
        print(f"\n{'='*60}")
        print(f"Training {fold_name} — HRNet-W32")
        print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("PER-FOLD RESULTS — HRNet-W32")
    print(f"{'='*60}")
    for row in summary_rows:
