#!/usr/bin/env python3
"""
Train HeatmapNet with ResNet-101 backbone on DSAA Dental dataset (5-fold CV).
...
Usage:
  ...
  python train_resnet101.py --k-folds 5 --epochs 300 --batch-size 8

Outputs (per fold + overall):
  results/train_resnet101/<run_name>/
    fold_1/  ...  fold_5/
...
DEFAULT_SAVE_ROOT = DSAA_ROOT / "results" / "train_resnet101"
...
# ---------------------------------------------------------------------------
# Model — ResNet-101 backbone
# ResNet-101 encoder: (B, 2048, 16, 16) for 512x512 input (same spatial dims as ResNet-50).
# Decoder: 3× ConvTranspose2d upsample to (B, 256, 128, 128), then 1×1 head → (B, NUM_KPS, 128, 128).
# ---------------------------------------------------------------------------
class HeatmapNet(nn.Module):
    def __init__(self):
        super().__init__()
        backbone = models.resnet101(weights=models.ResNet101_Weights.DEFAULT)
        self.backbone = nn.Sequential(*list(backbone.children())[:-2])  # (B, 2048, 16, 16)
...
def plot_loss_curves(all_epochs_csv: Path, out_png: Path) -> None:
    ...
    fig.suptitle("ResNet-101 — DSAA Dental", fontsize=12)
    fig.tight_layout()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train HeatmapNet (ResNet-101) on DSAA Dental — 5-fold CV.")
...
def main() -> None:
    ...
    for i in range(1, args.k_folds + 1):
        ...
        print(f"\n{'='*60}")
        print(f"Training {fold_name} — ResNet-101")
        print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("PER-FOLD RESULTS — ResNet-101")
    print(f"{'='*60}")
    for row in summary_rows:
        print(f"{row.get('fold')}: "
              f"best_val={row.get('best_val_loss', float('nan')):.4f} | "
              f"incisor_mm={row.get('incisor_sum_mm', float('nan')):.3f} | "
              f"left_arc_mm={row.get('left_arc_mm', float('nan')):.3f} | "
              f"right_arc_mm={row.get('right_arc_mm', float('nan')):.3f} | "
    ...
    if summary_rows:
        has_sum3 = all("sum_incisor_left_right_mm" in r and r["sum_incisor_left_right_mm"] != ""
