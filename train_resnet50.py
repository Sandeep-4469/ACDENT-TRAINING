#!/usr/bin/env python3
"""
Train HeatmapNet with ResNet50 backbone on DSAA Dental dataset (5-fold CV).
...
Usage:
  ...
  python train_resnet50.py --k-folds 5 --epochs 300 --batch-size 8

Outputs (per fold + overall):
  results/train_resnet50/<run_name>/
...
DEFAULT_SAVE_ROOT = DSAA_ROOT / "results" / "train_resnet50"
...
def _sort_arc_lines(lines: list) -> list:
    """Sort arc line pairs left-to-right so kp indices are always consistent.

    Maxilla (3 lines):  scale=lines[0], arcs=lines[1:3]  → kp 2,3=left  kp 4,5=right
    Mandible (7 lines): scale=lines[0], incisors=lines[1:5], arcs=lines[5:7]
                                                            → kp 10,11=left kp 12,13=right
    Without this ~50% of mandible annotations have arcs reversed, producing
    conflicting gradients that prevent the model from learning arc positions.
    """
    lines = list(lines)
    if len(lines) == 3:
...
class HeatmapDentalDataset(Dataset):
    ...
    def _generate_heatmap(self, pts: List[List[float]]) -> np.ndarray:
        ...
        sigma = getattr(HeatmapDentalDataset, "_sigma", 3)
...
# ---------------------------------------------------------------------------
# Model — ResNet50 or ConvNeXt-Tiny backbone
# ---------------------------------------------------------------------------
class HeatmapNet(nn.Module):
    def __init__(self, backbone_type: str = "resnet50"):
        ...
        if backbone_type == "convnext_tiny":
            ...
            self.backbone = bb.features          # [B, 768, 16, 16] for 512×512 input
            in_ch = 768
        else:
            bb = models.resnet50(weights=models.ResNet50_Weights.DEFAULT)
            self.backbone = nn.Sequential(*list(bb.children())[:-2])  # [B, 2048, 16, 16]
            in_ch = 2048
    ...
    with torch.no_grad():
        for imgs, _, gt_pts, keys in tqdm(loader, leave=False):
            ...
            for i in range(len(imgs)):
                ...
                if valid_kps == 6:
                    # _sort_arc_lines guarantees kp 2,3 = left arc, kp 4,5 = right arc
                    ...
                    # _sort_arc_lines guarantees kp 10,11 = left arc, kp 12,13 = right arc
...
def train_one_fold(
    ...
    if exclude_vlm:
        ...
        train_data = {k: v for k, v in train_data.items() if not _is_vlm(k)}
        test_data  = {k: v for k, v in test_data.items()  if not _is_vlm(k)}
        print(f"[{fold_name}] Excluded VLM: train {n_train_before}→{len(train_data)}, "
              f"test {n_test_before}→{len(test_data)}")
    ...
    with config_json.open("w", encoding="utf-8") as f:
        json.dump({
            ...
            "exclude_vlm":      exclude_vlm,
        }, f, indent=2)

    with log_csv.open("w", newline="") as f:
        csv.writer(f).writerow(["epoch", "train_loss", "val_loss", "learning_rate"])

    model = HeatmapNet(backbone_type).to(DEVICE)
    if pretrained_path and Path(pretrained_path).exists():
        model.load_state_dict(torch.load(pretrained_path, map_location=DEVICE))
        print(f"[{fold_name}] Loaded pretrained weights from {pretrained_path}")
    ...
    best_model = HeatmapNet(backbone_type).to(DEVICE)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train HeatmapNet (ResNet50) on DSAA Dental — 5-fold CV.")
    ...
    parser.add_argument("--pretrained",   type=str,   default="",
                        help="Path to .pth checkpoint to fine-tune from")
    parser.add_argument("--epochs",       type=int,   default=EPOCHS)
    parser.add_argument("--batch-size",   type=int,   default=BATCH_SIZE)
    parser.add_argument("--exclude-vlm",  action="store_true",
                        help="Exclude VLM-generated images (keys matching __*__) from train and test")
    # Loss weight overrides
    parser.add_argument("--lr",               type=float, default=LR)
    parser.add_argument("--coord-loss-w",     type=float, default=COORD_LOSS_W)
    parser.add_argument("--length-loss-w",    type=float, default=LENGTH_LOSS_W)
    parser.add_argument("--arc-side-w",       type=float, default=ARC_SIDE_LOSS_W)
    parser.add_argument("--arc-center-w",     type=float, default=ARC_CENTER_ALIGN_W)
    parser.add_argument("--arc-contrast-w",   type=float, default=ARC_CONTRAST_W)
    parser.add_argument("--sigma",            type=int,   default=3,
                        help="Gaussian sigma for heatmap generation")
    parser.add_argument("--softargmax-temp",  type=float, default=SOFTARGMAX_TEMP)
...
def main() -> None:
    ...
    global EPOCHS, BATCH_SIZE, LR, COORD_LOSS_W, LENGTH_LOSS_W
    global ARC_SIDE_LOSS_W, ARC_CENTER_ALIGN_W, ARC_CONTRAST_W, SOFTARGMAX_TEMP
    EPOCHS             = args.epochs
    BATCH_SIZE         = args.batch_size
    LR                 = args.lr
    COORD_LOSS_W       = args.coord_loss_w
    LENGTH_LOSS_W      = args.length_loss_w
    ARC_SIDE_LOSS_W    = args.arc_side_w
    ARC_CENTER_ALIGN_W = args.arc_center_w
    ARC_CONTRAST_W     = args.arc_contrast_w
    SOFTARGMAX_TEMP    = args.softargmax_temp
    # Sigma is passed per-dataset-instance via monkey-patch on the class
    HeatmapDentalDataset._sigma = args.sigma
    ...
    for i in range(1, args.k_folds + 1):
        ...
        print(f"\n{'='*60}")
        print(f"Training {fold_name} — {args.backbone}")
        print(f"{'='*60}")
        metrics = train_one_fold(fold_name, fold_root, run_root, seed=args.seed + i,
                                 exclude_vlm=args.exclude_vlm, backbone_type=args.backbone,
    ...
    # Consolidate all per-fold epoch logs
    ...
    print(f"\n{'='*60}")
    print("PER-FOLD RESULTS — ResNet50")
    print(f"{'='*60}")
    for row in summary_rows:
